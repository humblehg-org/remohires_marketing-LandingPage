...
/** Mirrors the source's inline onchange that toggled a `.has-val` class on
 * the Industry select — its label has no `required` peer to key CSS off
 * of, so JS has to do it instead. */
function toggleHasValue(target: HTMLSelectElement) {
  target.closest(`.${styles.floatingGroup}`)?.classList.toggle(styles.hasValue, target.value.length > 0);
}
...
export function MatchForm({ onClose }: { onClose: () => void }) {
    ...
    try {
      ...
      if (data && data.success) {
        if (!trackedRef.current) {
          ...
          window.dataLayer.push({ event: "generate_lead", value: 49, currency: "USD" });
        ...
        router.push("/chatgpt-automation/thank-you");
        return;
      } else {
  ...
  return (
    <div className={styles.formCard}>
      ...
      <h3 className={styles.modalTitle}>Start your 14-day sprint</h3>
