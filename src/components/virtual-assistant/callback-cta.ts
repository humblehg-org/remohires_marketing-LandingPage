...
/** Every "Talk to Our Team" entry point (header, hero, mobile menu, mobile FAB,
 * mid-page and final CTA) dispatches this instead of holding its own copy of
 * the open/close state — <CallbackModal> is the only thing that listens. */
export const OPEN_CALLBACK_MODAL_EVENT = "virtual-assistant:open-callback-modal";

/**
 * Shared click handler for every booking CTA on /virtual-assistant. Replaces
...
export function handleCallbackClick(ctaName: string) {
  return (e: MouseEvent<HTMLAnchorElement>) => {
    e.preventDefault();
    sendGTMEvent({ event: "booking_cta_click", cta_name: ctaName, page_path: "/virtual-assistant" });
