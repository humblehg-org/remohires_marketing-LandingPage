    ...
    try {
      ...
      if (data && data.success) {
        if (!trackedRef.current) {
          ...
          window.dataLayer.push({ event: "lead_submit", form_location: "ai-insurance" });
        ...
        router.push("/ai-insurance/thank-you");
        return;
      } else {
  ...
  return (
    <div ref={cardRef} className={`formcard rev${revealed ? " in" : ""}`}>
      <form noValidate onSubmit={handleSubmit}>
        ...
        <input type="hidden" name="from_name" value="RemoHires Landing - AI Insurance" />
        <input
          type="checkbox"
          name="botcheck"
          className="hp"
        ...
        <div className="grid2">
          ...
          <div>
            ...
            <input id={`${idPrefix}-agency`} name="agency" type="text" required suppressHydrationWarning />
          </div>
        </div>
        <div className="grid2">
          <div>
        ...
        <label htmlFor={`${idPrefix}-ams`}>Your AMS (EZLynx, AMS360, HawkSoft, …)</label>
        <input id={`${idPrefix}-ams`} name="ams" type="text" suppressHydrationWarning />
        <label htmlFor={`${idPrefix}-message`}>Your biggest bottleneck right now</label>
        <textarea
          id={`${idPrefix}-message`}
          name="message"
          rows={2}
          placeholder="e.g. quotes going cold, double entry into QuickBooks…"
