...
import { Logo } from "@/components/business-automation/logo";
import { HeaderShadow } from "@/components/business-automation/header-shadow";
import { NavActive } from "@/components/business-automation/nav-active";
import { MobileNav } from "@/components/business-automation/mobile-nav";
import { StartLink } from "@/components/business-automation/start-link";
import { StartModal } from "@/components/business-automation/start-modal";
import { ScrollDepth } from "@/components/business-automation/scroll-depth";
import { Reveal } from "@/components/business-automation/reveal";
import { ChoreographedReveal } from "@/components/business-automation/choreographed-reveal";
import { ThemeToggle } from "@/components/business-automation/theme-toggle";
...
export const metadata: Metadata = {
  title: "Clear your team's backlog without hiring | RemoHires",
  description:
...
const BEFORE_BARS = ["70%", "90%", "50%", "80%"];

const AFTER_ROWS = [
  { kind: "automated" as const, width: "50%" },
  { kind: "automated" as const, width: "70%" },
  { kind: "team" as const, width: "40%" },
];

export default function BusinessAutomationPage() {
  return (
    <div id="business-automation-page" className={`${styles.page} ${inter.className} ${jetbrainsMono.variable}`}>
      <ScrollDepth />
      <StartModal />
      <HeaderShadow />
      <NavActive />
      ...
      {/* ===================== NAVBAR ===================== */}
      <header id="business-automation-header" className={styles.header}>
        ...
        <Reveal as="section" id="top" className={styles.hero}>
          <div className={styles.heroGrid}>
            <div className={styles.heroCopy}>
              <h1 className={styles.heroHeading}>Clear your team&rsquo;s backlog without hiring.</h1>
              <p className={styles.heroLead}>
                We find one repetitive process overloading your team, automate it using your existing tools, and
                have it running within 14 days.
              </p>
            ...
            <div className={styles.heroVisual}>
              <div className={styles.mockCard}>
                ...
                <div className={styles.mockBefore}>
                  <div className={styles.mockBeforeBars}>
                    {BEFORE_BARS.map((width, i) => (
                      <div key={i} className={styles.mockBar} style={{ width }} />
                    ))}
                  </div>
                  <div className={styles.mockBeforeMeta}>
                    <p className={styles.mockBeforeLabel}>Before</p>
                    <p className={styles.mockBeforeSub}>Team Overloaded</p>
                  </div>
                </div>

                <div className={styles.mockAfter}>
                  <div className={styles.mockAfterRows}>
                    {AFTER_ROWS.map((row, i) => (
                      <div key={i} className={row.kind === "team" ? styles.mockAfterRowMuted : styles.mockAfterRow}>
                        {row.kind === "automated" ? (
                          <span className={styles.mockCheckIcon}>
                            <CheckIconBold />
                          </span>
                        ) : (
                          <span className={styles.mockPeopleIcon}>
                            <PeopleIcon />
                          </span>
                        )}
                        <div
                          className={row.kind === "automated" ? styles.mockAfterBar : styles.mockAfterBarMuted}
                          style={{ width: row.width }}
                        />
                        <span className={row.kind === "automated" ? styles.mockAfterTag : styles.mockAfterTagMuted}>
                          {row.kind === "automated" ? "Automated" : "Team handles"}
                        </span>
                      </div>
                    ))}
                  </div>
